...
'''
### Create test cases ###
img1 = Image.open('dataset/samples/livingroom1.png').convert('L')
img2 = img1.rotate(10)

img1 = transforms.ToTensor() (img1).unsqueeze(dim=0).cuda()
img2 = transforms.ToTensor() (img2).unsqueeze(dim=0).cuda()

# Pair of different images, pair of same images
input1 = torch.cat([img1, img2])
input2 = torch.cat([img2, img2])

MI = MutualInformation(num_bins=256, sigma=0.1, normalize=True).cuda()
